...
<jsp:include page="/WEB-INF/views/common/nav.jsp">
	<jsp:param value="전자결재" name="pageTitle" />
	<jsp:param value="관리자 추가" name="selectMenu" />
</jsp:include>
<style>
...
function fn_admin_add() {
	...
	$("#autocomplete").keydown(function(key){
		...
		if(userName != "") {
          $.ajax({
                 type: 'post',
                 url: "${pageContext.request.contextPath}/approval/admin/searchAdmin",
                 dataType: "json",
	...
	$(document).on('click', '.memberList', function(){
		...
		var bool = confirm("["+userName+"] 회원을 전자결재 관리자로 추가시키겠습니까?");
		var bool2 = true;
		if(bool == true){
			<c:forEach var="v" items="${approvalAdminList }">
				if(userId == "${v.USERID}"){
					alert("이미 추가된 회원입니다.");
					bool2 = false;
				}			
			...
			if(bool2 == true){
				location.href="${pageContext.request.contextPath}/approval/admin/adminInsertEnd.do?userId="+userId;
			}
		}
	})
...
<table class="table" id="admin-table">
	...
	<tbody>
		<c:if test="${fn:length(approvalAdminList) != 0 }">
			<c:forEach var="v" items="${approvalAdminList }">
				<tr>
					...
					<td id="delete"><c:if test="${v.GRADE eq '슈퍼관리자' }">
							...
							<span id="delete-possible"
								onclick="location.href='${pageContext.request.contextPath}/approval/admin/approvalAdminDelete.do?userId=${v.USERID }&grade=${v.GRADE }'">삭제</span>
						</c:if></td>
				</tr>
			</c:forEach>
		</c:if>
