...
<jsp:include page="/WEB-INF/views/common/nav.jsp">
	<jsp:param value="게시판" name="pageTitle" />
	<jsp:param value="게시판 관리자 관리" name="selectMenu" />
</jsp:include>
<style>
...
function fn_admin_add() {
	...
	$("#autocomplete").keydown(function(key){
		...
		if(userName != "") {
          $.ajax({
                 type: 'post',
                 url: "${pageContext.request.contextPath}/board/admin/searchAdmin",
                 dataType: "json",
	...
	$(document).on('click', '.memberList', function(){
		...
		if(bool == true){
			...
			if(bool2 == true){
				location.href="${pageContext.request.contextPath}/board/admin/adminInsertEnd.do?userId="+userId;
			}
		}
	})
...
<table class="table" id="admin-table">
	...
	<tbody>
		<c:if test="${fn:length(boardAdminList) != 0 }">
			<c:forEach var="v" items="${boardAdminList }">
				<tr>
					...
					<td id="delete"><c:if test="${v.GRADE eq '슈퍼관리자' }">
							...
							<span id="delete-possible"
								onclick="location.href='${pageContext.request.contextPath}/board/admin/boardAdminDelete.do?userId=${v.USERID }&grade=${v.GRADE }'">삭제</span>
						</c:if></td>
				</tr>
			</c:forEach>
		</c:if>
